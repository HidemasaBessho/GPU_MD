...
__global__ void calc_force_kernel(double*x_dev,double*y_dev,double *fx_dev,double *fy_dev,double *a_dev,double *L_dev,int *list_dev,double *P_dev){
  ...
  if(i_global<NP){
    ...
    for(int j = 1; j<=list_dev[NN*i_global]; j++){
      dx = x_dev[i_global]-x_dev[list_dev[NN*i_global+j]];
      dy = y_dev[i_global]-y_dev[list_dev[NN*i_global+j]];
      dx -= (*L_dev)*floor(dx/(*L_dev)+0.5);
      dy -= (*L_dev)*floor(dy/(*L_dev)+0.5);	
      dr = sqrt(dx*dx+dy*dy);
      a_ij = 0.5*(a_dev[i_global]+a_dev[list_dev[NN*i_global+j]]);
      if(dr < a_ij){
	      dU_r = -(1.0-dr/a_ij)/a_ij; //derivertive of U wrt r.
	      fx_dev[i_global] -= dU_r*dx/dr;
	      fy_dev[i_global] -= dU_r*dy/dr;
        P_dev[i_global] -= dr*dU_r/(*L_dev)/(*L_dev)/2.0;
      }
    }
...
__global__ void volume_affine_step1_kernel(double *phi_dev,double *deltaphi_dev,double *L_dev,double *phi0_dev){
...
__global__ void volume_affine_step2_kernel(double *x_dev, double *y_dev,double *phi_dev,double *phi0_dev){
  int i_global = threadIdx.x + blockIdx.x*blockDim.x;
  if(i_global<NP){
    x_dev[i_global] = x_dev[i_global]*sqrt(*phi0_dev/(*phi_dev));
    y_dev[i_global] = y_dev[i_global]*sqrt(*phi0_dev/(*phi_dev));
  }
}
...
__global__ void rec(double *phi_dev,double *phi_rec_dev,double *P_dev,double *P_rec_dev){
  static int count=0;
  phi_rec_dev[count] = *phi_dev;
  P_rec_dev[count] = P_dev[0];
  printf("%d,%.5f,%.10f\n",count,phi_rec_dev[count],P_rec_dev[count]);
  count++;
}

__global__ void check_deltaphi(double *deltaphi_dev,double *pot_dev,int *phiJ_flag_dev){
  if((pot_dev[0]/NP)>1.e-8){
    *deltaphi_dev = 1.e-4;
  }
  else{
    *deltaphi_dev = 1.e-6;
  }
  if((pot_dev[0]/NP)<1.e-16){
    *phiJ_flag_dev = 1;
  }
  else{
    *phiJ_flag_dev = 0;
...
int main(){
  double *x,*vx,*y,*vy,*fx,*fy,*pot,*a,*x_dev,*vx_dev,*y_dev,*dx_dev,*dy_dev,*vy_dev,*pot_dev,*a_dev,*fx_dev,*fy_dev,*power_dev,*L_dev,*P_dev;
  double *dt_dev,*alpha_dev,*phi_dev,*deltaphi_dev,deltaphi=1.e-3,phi=0.8,*phi0_dev;
  double *phi_rec_dev,*phi_rec,*P_rec_dev,*P_rec;
  int *list_dev,*map_dev,*gate_dev,*remain_dev,*reduce_dev;
  int *FIRE_gate_dev,FIRE_gate,*FIRE_param_gate_dev,*phiJ_flag_dev,phiJ_flag;
  int clock=0,step=0;
  ...
  phi_rec  = (double*)malloc(10000*sizeof(double));
  P_rec  = (double*)malloc(10000*sizeof(double));
  ...
  cudaMalloc((void**)&P_dev,  NB * NT * sizeof(double));
  cudaMalloc((void**)&P_rec_dev,  10000 * sizeof(double));
  cudaMalloc((void**)&phi_rec_dev,  10000 * sizeof(double));
  ...
  cudaMalloc((void**)&phi0_dev, sizeof(double));
  ...
  cudaMalloc((void**)&phiJ_flag_dev, sizeof(int));
  ...
  cudaMemcpy(phi0_dev,&phi,sizeof(double),cudaMemcpyHostToDevice);
  cudaMemcpy(deltaphi_dev,&deltaphi,sizeof(double),cudaMemcpyHostToDevice);
  ...
  while(phi<=phi_max){
    ...
    for(;;){
      ...
      if(FIRE_gate == 1){
        ...
        reduce=NP/2,remain=NP-NP/2;
        while(reduce>0){
          add_reduction<<<(reduce+NT-1)/NT,NT>>>(P_dev,reduce_dev,remain_dev);
          reduce = remain/2;remain-=reduce;
          len_div<<<1,1>>>(reduce_dev,remain_dev);
        }
    ...
    phi += deltaphi;
    volume_affine_step1_kernel<<<1,1>>>(phi_dev,deltaphi_dev,L_dev,phi0_dev);
  ...
  for(;;){
    ...
    check_deltaphi<<<1,1>>>(deltaphi_dev,pot_dev,phiJ_flag_dev);
    cudaMemcpy(&phiJ_flag,phiJ_flag_dev,sizeof(int),cudaMemcpyDeviceToHost);
    if(phiJ_flag==1){
  ...
  cudaMemcpy(phi_rec,phi_rec_dev,sizeof(double),cudaMemcpyDeviceToHost);
  cudaMemcpy(P_rec,P_rec_dev,sizeof(double),cudaMemcpyDeviceToHost);
  //for(int i=0;i<step;i++){
    //cout << phi_rec[i] << " " << P_rec[i] << endl;
  //}
  sec = measureTime()/1000.;
  cout<<"time(sec):"<< sec <<endl;
  cudaFree(x_dev);
